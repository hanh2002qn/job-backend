...
export class TopCvCrawler implements JobCrawlerStrategy {
        ...
        if (url && title) {
          ...
          if (!existing) {
            try {
              ...
              this.logger.error(
                `Failed to fetch details for ${url}`,
                err.message,
              );
            }
          }
  ...
  private async fetchTopCVDetail(url: string) {
    ...
    const quantity = quantityRaw
      ? parseInt(quantityRaw.match(/\d+/)?.[0] || '0')
      : 0;
    const experienceLevel = findGeneralInfo('Kinh nghiệm');

    // Deadline
    let deadline: Date | null = null;
    const deadlineText = $('.job-detail__info--deadline')
      .text()
    ...
    const logoUrl =
      ...
      $('.box-company-logo img').attr('src');
    const companyAddress =
      $('.company-address').text().trim() ||
      $('.box-company-address').text().trim();
